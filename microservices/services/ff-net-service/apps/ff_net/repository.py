...
class BaseRepository:
    ...
    def find_all(self):
        ...
        No WHERE/LIMIT, so no ALLOW FILTERING is needed. The driver's ResultSet
        pages transparently as it's iterated, so this is safe for tables larger
        than one page without any manual paging-state handling.
        """
        cql = f"SELECT * FROM {self.keyspace}.{self.table}"
        result = self._execute(cql)
